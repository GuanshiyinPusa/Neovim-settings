		...
		config = function()
			...
			conform.setup({
				...
				formatters = {
					...
					isort = {
						-- must match black's style to avoid conflicts on every save
						prepend_args = { "--profile", "black", "--line-length", "88" },
					},
					black = {
						-- no --fast: keeps AST equivalence check so bad rewrites are caught
						prepend_args = { "--line-length", "88", "--target-version", "py310" },
					},
					prettier = {
